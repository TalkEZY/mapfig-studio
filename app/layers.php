<?PHP
	...
	$error   = "";
	$success = "";
	if(strlen($_SESSION['response']['layers']['error']) > 0){
		$error = '<div ng-show="authMsg" class="alert alert-danger text-center ng-binding">'.$_SESSION['response']['layers']['error'].'</div>';
	}
	else if(strlen($_SESSION['response']['layers']['success']) > 0){
		$success = '<div ng-show="authMsg" class="alert alert-success text-center ng-binding">'.$_SESSION['response']['layers']['success'].'</div>';
	}
	unset($_SESSION['response']['layers']['error']);
	unset($_SESSION['response']['layers']['success']);
	
	$PAGE = "VIEW_LAYERS";
	$user = $_SESSION['user'];
	
	if((int)$_GET['id'] > 0) {
		deleteLayers($user['id'], (int)$_GET['id']);
		redirect("layers.php");
	}
	
	if((int)$_GET['groupid'] > 0){
		$layers = getLayersByGroupId($user['id'], (int)$_GET['groupid']);
	}
	else{
            ...
            <div class="">
               <div class="col-lg-12">
                  <div class="panel panel-default">
                     <div class="panel-heading">Base Layers</div>
                     <div class="panel-body">
                        <div class="table-responsive">
							<div style="height:400px;">
								  ...
								  <tbody>
									<?PHP foreach($layers as $layer) { ?>
									 <tr>
										...
										<td><?=$layer['lkey']?></td>
										<td>
											<a href="layers-edit.php?id=<?=$layer['id']?>" class="btn btn-sm btn-info"><i class="fa fa-edit"></i> Edit</a>
											<a href="layers.php?id=<?=$layer['id']?>" class="btn btn-sm btn-danger"><i class="fa fa-remove"></i> Delete</a>
										</td>
									 </tr>
									<?PHP } ?>
								  </tbody>
								...
								<?PHP
									if(count($layers) == 0){
										echo "<p style='text-align:center;'><br><br><br>No data Found!</p>";
									}
								?>
							</div>
                        </div>
