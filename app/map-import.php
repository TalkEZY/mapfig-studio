<?PHP
	...
	$error   = "";
	$success = "";
	if(strlen($_SESSION['response']['map-import']['error']) > 0){
		$error = '<div ng-show="authMsg" class="alert alert-danger text-center ng-binding">'.$_SESSION['response']['map-import']['error'].'</div>';
	}
	else if(strlen($_SESSION['response']['map-import']['success']) > 0){
		$success = '<div ng-show="authMsg" class="alert alert-success text-center ng-binding">'.$_SESSION['response']['map-import']['success'].'</div>';
	}
	unset($_SESSION['response']['map-import']['error']);
	unset($_SESSION['response']['map-import']['success']);
                  ...
                  <div class="panel panel-default">
                     ...
                     <div class="panel-body">
                        <div class="table-responsive">
							<form action="processor/map-import/index.php" method="post" enctype="multipart/form-data">
								<table class="table">
								  <tbody>
									 <tr>
										<td>Upload File</td>
										<td><input type="file" name="sourceUpload" class="form-control" accept=".kml, .json, .csv, .gpx"></td>
										<td><small>(.kml, .json, .csv, .gpx)</small></td>
									 </tr>
								  </tbody>
								</table>
								<br/>
								<div class="pull-right">
									<button name="upload" class="btn btn-success"><i class="fa fa-upload"></i> Upload</button>
								</div>
							</form>
                        </div>
                     </div>
