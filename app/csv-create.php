<?PHP
	...
	$data = array(array("Location", "Pop-Up Content", "Latitude", "Longitude"));
	if($_SESSION['csv']['data']) {
		$data = $_SESSION['csv']['data'];
	}
	
	$error   = "";
	
	if (strlen($_SESSION['response']['csv-create']['error']) > 0){
		$error = '<div ng-show="authMsg" class="alert alert-danger text-center ng-binding">'.$_SESSION['response']['csv-create']['error'].'</div>';
	}
	unset($_SESSION['response']['csv-create']['error']);
	
	$PAGE = "CSV_CREATE";
	$title = "Create Map with Table";
?>
<!DOCTYPE html>
<html lang="en">
               ...
               <div class="col-lg-12">
                  <div class="panel panel-default">
					 <div class="panel-heading">
						Add/Edit/Delete Columns/Rows and click "Create" (Note: Latitude/Longitude are required)
						<br/>
                     ...
                     <div class="panel-body">
						...
						<div class="col-lg-12">
							<div class="pull-right">
								<form action="processor/csv-create/index.php" method="post">
									<input type="hidden" name="csv_data" id="csv_data"/>
									<input type="hidden" name="defaultProperty" id="defaultProperty"/>
									
									<input type="hidden" name="stylingColumn"/>
									<input type="hidden" name="shapeStyling" id="shapeStyling" value="[]"/>
									
									<input type="hidden" name="filteredColumns"/>
									...
									<button id="create" class="btn btn-success"><i class="icon-map"></i> Create</button>
								</form>
							</div>
                        </div>
                     </div>
   ...
   <script src="js/map.csv.js"></script>
	...
	<script>
		...
		$(document).ready(function(){
			<?PHP
				if(count($data) > CSV_ROWS_LIMIT+1) {
					echo 'Alert("Max allowed rows are '.CSV_ROWS_LIMIT.'", "warning")';
				}
			?>
